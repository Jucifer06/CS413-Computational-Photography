...
Change the dataset creation process: concentrate only on car (and later street) scenes generation. So the descrption stays mostly the same, with some color changes, and more importantly seed changes (the scene can be described the same, but changing the seed will change the style, the surroundings,...).
